...
public class ScheduleDaoImpl extends JdbcDaoSupport{
	...
	public List<Schedule> getByScheduleId(String id, String begtime, String endtime){
		String[] args={id, begtime, endtime};
	...
	public List<Programme> getProgramsByStationID(String sid){
		String[] args = {sid};
		String sql = "select distinct program.program_id, schedule.station_id, program.language, program.title, program.genre_desc1, program.genre_desc2, " +
				"program.genre_desc3, program.genre_desc4, program.genre_desc5, program.description1, program.description2 " +
				"from program, schedule where schedule.station_id = ? and schedule.program_id = program.program_id";
		List<Programme> programs = this.getJdbcTemplate().query(sql, args,
            ...
            public List<Programme> extractData(ResultSet rs)  
                ...
                while (rs.next()) {  
                	...
                	program.stationID = rs.getString("STATION_ID");
	...
	public List<Lineup> getLineupByStationID(String sid){
		String[] args = {sid};
		String sql = "select lineup_id, station_id, headend_id from lineup where station_id = ?";
		List<Lineup> lineups = this.getJdbcTemplate().query(sql, args,
				new ResultSetExtractor<List<Lineup>>() {  
            @Override  
            public List<Lineup> extractData(ResultSet rs)  
                    throws SQLException, DataAccessException {  
                List<Lineup> lineupList = new ArrayList<Lineup>();  
                while (rs.next()) {  
                	Lineup lineup = new Lineup();  
                	lineup.lineupID = rs.getString("LINEUP_ID");  
                	lineup.stationID = rs.getString("STATION_ID");
                	lineup.headendID = rs.getString("HEADEND_ID");
	...
	public List<Headend> getHeadendByStationID(String sid){
		String[] args = {sid};
		String sql = "select lineup.station_id, headend.headend_id, headend.headend_name, headend.state, headend.headend_location from lineup, " +
						"headend where lineup.station_id = ? and lineup.headend_id = headend.headend_id";
		List<Headend> headends = this.getJdbcTemplate().query(sql, args,
            ...
            public List<Headend> extractData(ResultSet rs)  
                ...
                while (rs.next()) {  
                	...
                	headend.stationID = rs.getString("STATION_ID");
	...
	public List<Station> getStationByStationID(String sid){
		String[] args = {sid};
		String sql = "select station_id, station_name, country, state, city from station where station_id = ?";
...
/*		Resource resource = new ClassPathResource("applicationContext.xml");
		...
		DriverManagerDataSource dataSource = new DriverManagerDataSource(
				"jdbc:oracle:thin:@150.236.80.37:1521:ttv","epgmanager","epgmanager");
